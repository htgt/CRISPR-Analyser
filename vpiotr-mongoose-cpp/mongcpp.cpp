...
{
  const char *request_method = getInfo()->request_method;
...
{
  const char *http_version = getInfo()->http_version;
...
{
  const char *remote_user = getInfo()->remote_user;
  if (remote_user != NULL)
    return std::string(remote_user);
  else
    return "";
}
